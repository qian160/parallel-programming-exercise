...
int array_sum_int_avx2_aligned(int *array, int n){
	assert((n & 0b111) == 0);
	int sum = 0;
...
int array_sum_int_avx2_unaligned(int *array, int n){
	assert((n & 0b111) == 0);
	int sum = 0;
...
int main()
{
	...
	for(int i = 0; i < 1024; i++)
		a[i] = b[i] = 1;
	asm_test();
	printf("sum of a = %d\n", array_sum_int_avx2_aligned(a, 1024));
	printf("sum of a = %d\n", array_sum_int_avx2_unaligned(a, 1024));
}
